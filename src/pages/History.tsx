                            ...
                            <CardContent className="p-0">
                                ...
                                >
                                    ...
                                    <div className="w-full md:w-32 flex flex-col items-center justify-center p-6 bg-slate-50 border-b md:border-b-0 md:border-r border-slate-100 group-hover:bg-primary/5 transition-colors">
                                        <span className="text-2xl font-black text-primary">{entry.readinessScore}%</span>
                                    ...
                                    <div className="flex-1 p-6 space-y-4">
                                        <div className="flex items-start justify-between">
                                            <div className="space-y-1">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <h3 className="font-bold text-slate-900 text-lg">{entry.company || 'Unknown Company'}</h3>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    <Briefcase className="h-4 w-4 text-slate-400" />
                                                    <p className="text-slate-500 font-medium">{entry.role || 'SDE-1'}</p>
                                                </div>
                                            </div>
                                            <div className="text-right flex flex-col items-end gap-2">
                                                <div className="flex items-center gap-1.5 text-xs font-bold text-slate-400 bg-white border border-slate-100 px-3 py-1 rounded-full">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {entry.extractedSkills.flatMap(s => s.skills).slice(0, 5).map((skill, i) => (
                                            ...
                                            {entry.extractedSkills.flatMap(s => s.skills).length > 5 && (
                                                <span className="text-[10px] font-bold text-primary px-2 py-0.5">+ {entry.extractedSkills.flatMap(s => s.skills).length - 5} more</span>
                                            )}
                                        </div>
                                    </div>
